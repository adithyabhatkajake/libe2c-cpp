...
void E2CCore::update_hqc(const block_t &_hqc, const quorum_cert_bt &qc) {
...
void E2CCore::on_receive_vote(const Vote &vote) {
...
void E2CCore::prune(uint32_t staleness) {
    block_t start;
    /* skip the blocks */
...
promise_t E2CCore::async_qc_finish(const block_t &blk) {
...
void E2CCore::on_qc_finish(const block_t &blk) {
    auto it = qc_waiting.find(blk);
    if (it != qc_waiting.end())
    {
        it->second.resolve();
...
promise_t E2CCore::async_hqc_update() {
